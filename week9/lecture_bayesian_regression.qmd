...
```{r}
ggplot(data = bikes, aes(x = date, y = rides, color = weekend, shape = holiday)) +
  geom_point()
```
...
```{r}
ggplot(data = bikes, aes(x = day_of_the_week, y = rides, fill = as.factor(year)) +
  geom_col() +
  facet_wrap(~season)
```
